...
from monai.transforms import (Compose, NormalizeIntensityd, ScaleIntensityd, RandCropByPosNegLabeld,
...
PARAMS = dict(
    epochs = 1000,
    exp_name = 'xunet_pos_weight100_lr1e-6',
    ...
    scheduler_params = dict(max_lr=1e-6, div_factor=10, final_div_factor=100, pct_start=0.1),
...
def loss_function(outputs, labels, masks):
    loss = BCEWithLogitsLoss(pos_weight=torch.tensor([100],dtype=outputs.dtype).to(outputs.device))
...
def get_train_transforms():
    return Compose(
        [
            NormalizeIntensityd(keys="volume_npy",
                                nonzero=True,
                                channel_wise=True),
            # ScaleIntensityd(keys="volume_npy"),
            RandCropByPosNegLabeld(keys=("volume_npy", "mask_npy", "label_npy"),
                                   label_key="label_npy",
                                   spatial_size=PARAMS["patch_size"],
                                   num_samples=PARAMS["num_samples"],
                                   image_key="volume_npy",
                                   image_threshold=0),
            RandAffined(keys=("volume_npy", "mask_npy", "label_npy"),
                        prob=0.75,
                        rotate_range=(np.pi / 4, np.pi / 4),
                        translate_range=(0.0625, 0.0625),
                        scale_range=(0.1, 0.1)),
            RandFlipd(keys=("volume_npy", "mask_npy", "label_npy"),
                      spatial_axis=0,
                      prob=0.5),
            RandFlipd(keys=("volume_npy", "mask_npy", "label_npy"),
                      spatial_axis=1,
                      prob=0.5),
            RandGaussianNoised(keys="volume_npy",
                               prob=0.15,
                               mean=0.0,
                               std=0.01),
            RandGaussianSmoothd(keys="volume_npy",
                                prob=0.15,
                                sigma_x=(0.5, 1.15),
                                sigma_y=(0.5, 1.15)),
            RandScaleIntensityd(keys="volume_npy",
                                factors=0.3,
                                prob=0.15),
            RandZoomd(keys=("volume_npy", "mask_npy", "label_npy"),
                      min_zoom=0.9,
                      max_zoom=1.2,
